...
class _CreateJobWizardState extends ConsumerState<CreateJobWizard> {
  ...
  void _applyManualPromo(String code) async {
    ...
    try {
      ...
      if (promo.applicableTo != 'services' && promo.applicableTo != 'both') {
        setState(() {
          _promoFeedback = 'This promo is not applicable to services.';
          _appliedPromo = null;
        });
        return;
      }
      ...
      if (promo.onlyForSubscribed && !user.isPremium) {
        setState(() {
          _promoFeedback =
              'This promo code is only for subscribed premium users.';
          _appliedPromo = null;
        });
        return;
      }
      ...
      if (promo.applicableRoles.isNotEmpty &&
          !promo.applicableRoles.contains('employer')) {
        setState(() {
          _promoFeedback = 'This promo is not applicable for employers.';
          _appliedPromo = null;
        });
        return;
      }
            ...
            children: [
              ...
              if (currentStep == 1) ...[
                          ...
                          if (categoryStep == 1) {
                            ...
                            return GestureDetector(
                              onTap: () {
                                ...
                                ref
                                        .read(
                                          categoryPickerStepProvider.notifier,
                                        )
                                        .state =
                                    2;
                              },
                ...
                UIHelpers.buildPrimaryButton("Next Step: Location", () {
                  ...
                  final hasProfanity =
                      checkProfanity(titleText) || checkProfanity(descText);
                    ...
                    children: ['On-site', 'Remote'].map((type) {
                      ...
                      return Expanded(
                        child: GestureDetector(
                          onTap: () =>
                              ref
                                      .read(workLocationTypeProvider.notifier)
                                      .state =
                                  type,
                          child: Container(
                            padding: const EdgeInsets.symmetric(vertical: 12),
  ...
  Future<void> _performSearch(String query) async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
          'https://nominatim.openstreetmap.org/search?format=json&q=${Uri.encodeComponent(trimmedQuery)}&limit=5',
        ),
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _errorMessage =
              "Failed to load locations. Code: ${response.statusCode}";
          _isLoading = false;
        });
      }
    } catch (e) {
