...
void main() {
  group('TransitRepository Collection & Flow Integration Tests', () {
    ...
    test('Verify booking request flow updates all target collections', () async {
    ...
    test('Verify approval, contract signature, and completion of vehicle rental', () async {
      ...
      final reqDoc = await firestore.collection('rental_requests').doc('req123').get();
      expect(reqDoc.data()!['status'], equals('Approved'));

      final escrowDoc = await firestore.collection('rental_escrows').doc('rental123').get();
      expect(escrowDoc.exists, isTrue);
      expect(escrowDoc.data()!['amount'], equals(2000.0));

      final deletedReqEscrow = await firestore.collection('rental_escrows').doc('req123').get();
      expect(deletedReqEscrow.exists, isFalse);

      final rentalDocAfterApproval = await firestore.collection('rentals').doc('rental123').get();
      expect(rentalDocAfterApproval.data()!['status'], equals('Awaiting Signature'));
      ...
      final finalRentalDoc = await firestore.collection('rentals').doc('rental123').get();
      expect(finalRentalDoc.data()!['status'], equals('Available'));
    });

    test('Verify property rental creation, booking & completion collections', () async {
      firestore.db['users/host123'] = {
        'name': 'Property Host',
        'email': 'host@tranyx.com',
        'accountType': 'nyxian',
    ...
    test('Verify submission of ratings writes to correct collections', () async {
      ...
      firestore.db['rental_history/rh123'] = {
        'id': 'rh123',
      };
      ...
      final propHistDoc = await firestore.collection('property_history').doc('ph123').get();
    ...
    test('Verify KYC submission collections', () async {
      ...
      final kycDoc = await firestore.collection('kyc_submissions').doc('user123').get();
