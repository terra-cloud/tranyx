...
class _ListPropertyModalState extends State<ListPropertyModalComponent> {
          ...
          [
            ...
            div(classes: 'sticky top-0 z-10 flex items-center justify-between p-6 border-b ${isDark ? "bg-zinc-900/90 border-zinc-800" : "bg-white/90 border-zinc-100"} backdrop-blur-md', [
              div([
                ...
                p(classes: 'text-sm ${isDark ? "text-zinc-400" : "text-zinc-500"}', [Component.text('Step $_step of 3')]),
              ]),
              button(
                classes: 'p-2 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors',
                events: {'click': (_) => component.appState.setState(() => component.appState.showListPropertyModal = false)},
                [lIcon('x', cls: 'w-6 h-6')],
              ),
            ]),
            ...
            div(classes: 'p-6 flex-1 space-y-6', [
              ...
              if (_step == 1) ...[
                ...
                div(classes: 'grid grid-cols-2 gap-4 mb-4', [
                  div([
                    label(classes: 'block text-sm font-semibold mb-2 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text('Property Category')]),
                    select(
                      classes: 'w-full p-3 rounded-xl border ${isDark ? "bg-zinc-900 border-zinc-700 text-white" : "bg-white border-zinc-300"} outline-none cursor-pointer',
                      events: {'change': (e) {
                        setState(() {
                          _selectedCategory = PropertyCategory.values.firstWhere((c) => c.name == (e.target as dynamic).value);
                  ...
                  div([
                    label(classes: 'block text-sm font-semibold mb-2 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text('Property Type')]),
                    select(
                      classes: 'w-full p-3 rounded-xl border ${isDark ? "bg-zinc-900 border-zinc-700 text-white" : "bg-white border-zinc-300"} outline-none cursor-pointer',
                      events: {'change': (e) {
                        setState(() {
                ...
                _inputField('Listing Title', _title, (v) => setState(() => _title = v), isDark, placeholder: 'e.g. Cozy 1BR Condo with Balcony in BGC'),
                
                div(classes: 'mb-4', [
                  label(classes: 'block text-sm font-semibold mb-2 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text('Description')]),
                  textarea(
                    classes: 'w-full p-3 rounded-xl border ${isDark ? "bg-zinc-900 border-zinc-700 text-white" : "bg-white border-zinc-300"} outline-none focus:border-purple-500 transition-colors h-28 resize-none',
                    attributes: {'placeholder': 'Provide details about the space, proximity to key locations, house rules...'},
                ...
                // Amenities Checklist
                div(classes: 'mb-4', [
                  label(classes: 'block text-sm font-semibold mb-3 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text('Amenities')]),
                  div(classes: 'grid grid-cols-2 md:grid-cols-3 gap-3', [
                    for (final amenity in _amenitiesList)
                      label(classes: 'flex items-center gap-2.5 p-3 rounded-xl border cursor-pointer transition-colors ${
                        ...
                        lIcon(
                          ...
                          amenity == 'Aircon' ? 'wind' : 
                          amenity == 'Parking' ? 'car' : 
                          amenity == 'Furnished' ? 'armchair' : 
                          amenity == 'Gym' ? 'dumbbell' : 'waves',
                          cls: 'w-4 h-4'
                        ),
                ...
                div(classes: 'grid grid-cols-2 gap-4', [
                  _inputField('Monthly Rent (Required)', _priceMonthly, (v) => setState(() => _priceMonthly = v), isDark, placeholder: '25000', type: InputType.number),
                  div([
                    label(classes: 'block text-sm font-semibold mb-2 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text('Security Deposit (Months)')]),
                    select(
                      classes: 'w-full p-3 rounded-xl border ${isDark ? "bg-zinc-900 border-zinc-700 text-white" : "bg-white border-zinc-300"} outline-none cursor-pointer',
                      events: {'change': (e) {
                        setState(() {
                  ...
                  _inputField('Weekly Rate (Optional)', _priceWeekly, (v) => setState(() => _priceWeekly = v), isDark, placeholder: 'e.g. 7000', type: InputType.number),
                  _inputField('Daily Rate (Optional)', _priceDaily, (v) => setState(() => _priceDaily = v), isDark, placeholder: 'e.g. 1500', type: InputType.number),
                ]),
                ...
                div(classes: 'mt-6 p-4 rounded-xl bg-purple-500/10 border border-purple-500/20 mb-6', [
                  div(classes: 'flex justify-between text-sm mb-2', [
                    span(classes: isDark ? 'text-zinc-400' : 'text-zinc-600', [Component.text('Platform Listing Fee (1.5% of Monthly)')]),
                    span(classes: 'font-bold text-purple-400', [Component.text('${_listingFee.toStringAsFixed(2)} TYX')]),
                  ]),
                  p(classes: 'text-xs ${isDark ? "text-zinc-500" : "text-zinc-400"}', [Component.text('To prevent listing spam, a small fee is deducted from your wallet to post your property.')]),
                ]),
                ...
                p(classes: 'text-sm mb-4 ${isDark ? "text-zinc-400" : "text-zinc-600"}', [Component.text('Upload 3 photos representing: Interior, Exterior, and details.')]),
                ...
                div(classes: 'mb-4', [
                  label(classes: 'block text-sm font-semibold mb-2 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text('Contract Lease Type')]),
                  select(
                    ...
                    [
                      option(value: 'Tranyx Standard', selected: _contractType == 'Tranyx Standard', [Component.text('Tranyx Standard Lease')]),
                      option(value: 'Custom Contract', selected: _contractType == 'Custom Contract', [Component.text('Custom Contract Terms')]),
                    ],
                  ),
                ]),
                ...
                if (_contractType == 'Tranyx Standard') ...[
                  div(classes: 'p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50', [
                    ...
                    p(classes: 'text-xs ${isDark ? "text-zinc-400" : "text-zinc-500"} mb-2', [
                      Component.text('By proceeding, you agree to compile under our standard P2P Rental Agreement underwritten by our partner legal firm.'),
                    ]),
                    if (_showPreview)
                      div(classes: 'mt-3 p-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg max-h-40 overflow-y-auto', [
                        p(classes: 'whitespace-pre-wrap text-xs ${isDark ? "text-zinc-400" : "text-zinc-600"} leading-relaxed', [
                          Component.text(buildDefaultPropertyContract(PropertyRental(
                            id: '',
                            hostId: '',
                  ...
                  div(classes: 'mt-4', [
                    label(classes: 'block text-sm font-semibold mb-2 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text('Custom Lease Terms')]),
                    textarea(
                      classes: 'w-full p-3 rounded-xl border ${isDark ? "bg-zinc-900 border-zinc-700 text-white" : "bg-white border-zinc-300"} outline-none focus:border-purple-500 transition-colors h-32 resize-none',
                      attributes: {'placeholder': 'Enter your custom property lease terms, house rules, water/electricity billing agreements...'},
            ...
            div(classes: 'p-6 border-t ${isDark ? "border-zinc-800" : "border-zinc-100"} flex items-center justify-between', [
              ...
              if (_step < 3)
                ...
                button(
                  classes: 'px-8 py-2 rounded-xl font-bold text-white bg-green-500 hover:bg-green-600 transition-colors flex items-center gap-2 border-0 cursor-pointer',
  ...
  Component _inputField(String labelText, String value, Function(String) onChange, bool isDark, {String? placeholder, InputType type = InputType.text}) {
    return div([
      label(classes: 'block text-sm font-semibold mb-2 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text(labelText)]),
      input(
  ...
  Component _photoUploadPlaceholder(String labelText, String currentUrl, int index, bool isDark) {
    ...
    return label(
      classes: 'relative aspect-video rounded-2xl border border-dashed flex flex-col items-center justify-center cursor-pointer overflow-hidden transition-colors ${isDark ? "border-zinc-800 hover:bg-zinc-800/50 bg-zinc-900/30" : "border-zinc-200 hover:bg-zinc-50 bg-zinc-50/50"}',
      attributes: {
      ...
      [
        ...
        if (currentUrl.isNotEmpty)
          ...
          p(classes: 'text-xs font-semibold ${isDark ? "text-zinc-400" : "text-zinc-500"}', [Component.text(labelText)]),
        ],

        if (isUploading)
          div(classes: 'absolute inset-0 bg-black/60 flex flex-col items-center justify-center text-white backdrop-blur-[2px] animate-fade-in', [
