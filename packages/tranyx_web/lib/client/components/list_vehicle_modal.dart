...
class _ListVehicleModalState extends State<ListVehicleModalComponent> {
  ...
  void _handleNext() {
    ...
    if (_step == 1) {
      ...
      if (_offersDriver) {
        if (_driverDailyPrice.trim().isEmpty || double.tryParse(_driverDailyPrice) == null || double.parse(_driverDailyPrice) < 0) {
          ...
          [
            ...
            div(classes: 'sticky top-0 z-10 flex items-center justify-between p-6 border-b ${isDark ? "bg-zinc-900/90 border-zinc-800" : "bg-white/90 border-zinc-100"} backdrop-blur-md', [
              ...
              button(
                ...
                events: {'click': (e) => component.appState.setState(() => component.appState.showListVehicleModal = false)},
                [lIcon('x', cls: 'w-6 h-6')],
              ),
            ]),
            ...
            div(classes: 'p-6 flex-1 space-y-6', [
              ...
              if (_step == 1) ...[
                ...
                div(classes: 'mb-4', [
                  label(classes: 'block text-sm font-semibold mb-2 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text('Vehicle Type')]),
                  select(
                ...
                div(classes: 'grid grid-cols-2 gap-4', [
                  ...
                  _inputField('Plate Number', _plateNumber, (v) => setState(() => _plateNumber = _formatPlateNumber(v)), isDark, placeholder: 'ABC-1234'),
                ]),
              ] else if (_step == 2) ...[
                // Step 2: Pricing & Location
                h3(classes: 'text-lg font-bold mb-4', [Component.text('Pricing & Location')]),
                div(classes: 'grid grid-cols-2 gap-4', [
                  _inputField('Daily Rate (Required)', _priceDaily, (v) => setState(() => _priceDaily = v), isDark, placeholder: '2500', type: InputType.number),
                  _inputField('12-Hour Rate', _price12h, (v) => setState(() => _price12h = v), isDark, placeholder: '1500', type: InputType.number),
                  _inputField('Weekly Rate', _priceWeekly, (v) => setState(() => _priceWeekly = v), isDark, placeholder: '15000', type: InputType.number),
                  _inputField('Monthly Rate', _priceMonthly, (v) => setState(() => _priceMonthly = v), isDark, placeholder: '50000', type: InputType.number),
                ]),
                div(classes: 'mt-4', [
                  _inputField('Late Penalty (Per Hour)', _extensionPenaltyPerHour, (v) => setState(() => _extensionPenaltyPerHour = v), isDark, placeholder: 'e.g. 200', type: InputType.number),
                ]),
                ...
                div(classes: 'mt-6 border-t ${isDark ? "border-zinc-800" : "border-zinc-200"} pt-6', [
                  ...
                  if (_offersDriver) ...[
                    div(classes: 'grid grid-cols-2 gap-4 mb-4', [
                      _inputField('Driver Daily Rate (TYX)', _driverDailyPrice, (v) => setState(() => _driverDailyPrice = v), isDark, placeholder: '500', type: InputType.number),
                      div(classes: '', [
                    ...
                    div(classes: 'mb-4', [
                      label(classes: 'block text-sm font-semibold mb-2 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text('Driver Notes / Details')]),
                      textarea(
                        classes: 'w-full p-3 rounded-xl border ${isDark ? "bg-zinc-900 border-zinc-700 text-white" : "bg-white border-zinc-300"} outline-none focus:border-purple-500 transition-colors',
                        rows: 3,
                        attributes: {'placeholder': 'e.g. Professional driver with 5+ years clean record, familiar with Metro Manila routes.'},
                ...
                div(classes: 'mt-6 border-t ${isDark ? "border-zinc-800" : "border-zinc-200"} pt-6', [
                  ...
                  p(classes: 'text-xs ${isDark ? "text-zinc-400" : "text-zinc-500"} mb-3', [
                    Component.text('Pan the map to the location of your vehicle and click "Confirm Site Location" to set it.')
                  ]),
                ...
                p(classes: 'text-sm mb-4 ${isDark ? "text-zinc-400" : "text-zinc-600"}', [Component.text('Tranyx requires 3 photos: Interior, Front, and Back.')]),
                ...
                div(classes: 'mb-4', [
                  label(classes: 'block text-sm font-semibold mb-2 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text('Contract Type')]),
                  select(
                    classes: 'w-full p-3 rounded-xl border ${isDark ? "bg-zinc-900 border-zinc-700 text-white" : "bg-white border-zinc-300"} outline-none focus:border-purple-500 transition-colors',
                    events: {'change': (e) => setState(() => _contractType = (e.target as dynamic).value)},
                    [
                      option(value: 'Tranyx Standard', selected: _contractType == 'Tranyx Standard', [Component.text('Tranyx Standard')]),
                      option(value: 'Custom Contract', selected: _contractType == 'Custom Contract', [Component.text('Custom Contract')]),
                    ],
                  ),
                ]),
                ...
                if (_contractType == 'Tranyx Standard') ...[
                  div(classes: 'p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50', [
                    ...
                    p(classes: 'text-xs ${isDark ? "text-zinc-400" : "text-zinc-500"} mb-2', [
                      Component.text('By proceeding, you agree to our standard P2P Rental Agreement underwritten by our partner legal firm.'),
                    ]),
                    if (_showPreview)
                      div(classes: 'mt-3 p-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg max-h-40 overflow-y-auto', [
                        p(classes: 'whitespace-pre-wrap text-xs ${isDark ? "text-zinc-400" : "text-zinc-600"} leading-relaxed', [
                          Component.text(buildDefaultTranyxContract(VehicleRental(
                            id: '',
                            hostId: '',
                  ...
                  div(classes: 'mt-4', [
                    label(classes: 'block text-sm font-semibold mb-2 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text('Custom Contract Terms')]),
                    textarea(
                    ...
                    div(classes: 'mt-3 flex items-center gap-3', [
                      span(classes: 'text-xs text-zinc-500 font-medium', [Component.text('Or upload a contract terms file:')]),
                      label(
                        classes: 'px-3 py-1.5 rounded-lg text-xs font-semibold bg-purple-600 hover:bg-purple-700 text-white cursor-pointer transition-colors',
  ...
  Component _selectField(
    ...
    return div([
      label(classes: 'block text-sm font-semibold mb-2 ${isDark ? "text-zinc-300" : "text-zinc-700"}', [Component.text(labelText)]),
      select(
        classes: 'w-full p-3 rounded-xl border ${isDark ? "bg-zinc-900 border-zinc-700 text-white" : "bg-white border-zinc-300"} ${isDisabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"} outline-none focus:border-purple-500 transition-colors',
        ...
        [
          ...
          for (final opt in optionsList)
            option(value: opt, selected: selectedValue == opt, [Component.text(opt)]),
        ],
      ),
    ]);
  }
