...
class _ManagePropertyModalState extends State<ManagePropertyModalComponent> {
          ...
          [
            ...
            div(classes: 'flex-1 overflow-y-auto p-6', [
              ...
              div(
                ...
                [
                  div([
                    p(classes: 'text-xs text-zinc-500 font-bold uppercase tracking-wider mb-1', [Component.text('Listing Status')]),
                    h3(classes: 'text-lg font-bold capitalize text-purple-400', [Component.text(status)]),
                  ]),
                  div(classes: 'text-right', [
                    p(classes: 'text-xs text-zinc-500 font-bold uppercase tracking-wider mb-1', [Component.text('Monthly Rent')]),
              ...
              if (status == 'Available') ...[
                if (_showConfirmDelete)
                  div(classes: 'mb-6 p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-sm flex flex-col gap-3', [
                    ...
                    p(classes: 'text-zinc-400 text-xs', [
                      Component.text('This will delete the property posting from Tranyx. The 1.5% listing fee is non-refundable.')
                    ]),
                    div(classes: 'flex items-center gap-2 mt-1', [
                      button(
                        classes: 'px-3 py-1.5 rounded-lg bg-red-500 text-white font-bold text-xs hover:bg-red-600 transition-colors border-0 cursor-pointer',
                        events: {'click': (_) => _deleteListing(prop['id'])},
                        disabled: _isProcessing,
                        [Component.text('Yes, Delete')],
                      ),
                      button(
                        classes: 'px-3 py-1.5 rounded-lg border border-zinc-705 text-zinc-400 hover:text-white font-bold text-xs transition-colors bg-transparent cursor-pointer',
                  ...
                  div(classes: 'mb-6 flex justify-end', [
                    button(
                      classes: 'px-4 py-2 text-xs font-bold text-red-500 border border-red-500/20 hover:bg-red-500/10 rounded-xl transition-all flex items-center gap-1.5 cursor-pointer bg-transparent',
                ...
                h3(classes: 'text-sm font-bold text-zinc-400 uppercase tracking-wider mb-4', [Component.text('Lease Booking Requests (${_requests.length})')]),
                ...
                if (_isLoadingRequests)
                  ...
                  div(classes: 'py-12 text-center rounded-2xl border-2 border-dashed ${isDark ? "border-zinc-800" : "border-zinc-200"}', [
                    ...
                    p(classes: 'text-xs text-zinc-500 mt-1', [Component.text('Rentees who request to rent your property will show up here.')]),
                  ])
                else
                    ...
                    for (final req in _requests)
                      div(
                        ...
                        [
                          ...
                          // Contract info
                          div(
                            classes: 'mb-4 p-3.5 rounded-xl text-xs ${isDark ? "bg-zinc-900 text-zinc-400" : "bg-zinc-50 text-zinc-650"}',
                            [
                          ...
                          div(classes: 'flex items-center justify-between gap-3 mb-4 pt-2 border-t ${isDark ? "border-zinc-800" : "border-zinc-100"}', [
                            button(
                              classes: 'text-xs font-bold text-indigo-400 hover:text-indigo-300 flex items-center gap-1 bg-transparent border-0 cursor-pointer',
                            ...
                            label(classes: 'flex items-center gap-2 cursor-pointer text-xs ${isDark ? "text-zinc-300" : "text-zinc-650"}', [
                              input(
                          ...
                          div(classes: 'flex items-center gap-2', [
                            button(
                              classes: 'flex-1 py-2 rounded-xl text-sm font-semibold text-white logo-gradient hover:opacity-90 disabled:opacity-50 transition-opacity border-0 cursor-pointer',
                              events: {'click': (_) => _approveRequest(req['id'])},
                              disabled: _isProcessing,
                              [Component.text('Approve & Await Signature')],
                            ),
                            button(
                              classes: 'px-4 py-2 rounded-xl text-sm font-semibold border ${isDark ? "border-zinc-800 hover:bg-zinc-900 text-zinc-400 hover:text-white" : "border-zinc-200 hover:bg-zinc-50 text-zinc-500"} transition-colors bg-transparent cursor-pointer',
                ...
                h3(classes: 'text-sm font-bold text-zinc-400 uppercase tracking-wider mb-4', [Component.text('Tenant & Lease Details')]),
                div(
                  ...
                  [
                    div(classes: 'flex items-center gap-3 mb-4', [
                      div(
                        ...
                        [
                          if (prop['renteePhotoUrl'] != null && prop['renteePhotoUrl'].toString().isNotEmpty && prop['renteePhotoUrl'].toString() != 'null')
                      ...
                      [
                        div(classes: 'flex justify-between', [
                          ...
                          span(classes: 'font-semibold', [
                            Component.text(DateTime.fromMillisecondsSinceEpoch(prop['startDate'] as int? ?? 0).toString().substring(0, 10))
                          ]),
                        ]),
                        div(classes: 'flex justify-between', [
                          span(classes: 'text-zinc-500', [Component.text('End Date')]),
                          span(classes: 'font-semibold', [
                            Component.text(DateTime.fromMillisecondsSinceEpoch(prop['endDate'] as int? ?? 0).toString().substring(0, 10))
                          ]),
                        ]),
                        div(classes: 'flex justify-between', [
                ...
                if (status == 'Awaiting Signature')
                  div(classes: 'p-4 rounded-xl border border-yellow-500/20 bg-yellow-500/10 text-yellow-500 text-xs text-center font-semibold mb-6', [
                    Component.text('Awaiting tenant to sign the contract and finalize this lease.')
                  ]),

                if (status == 'Booked' || status == 'Active')
                  button(
                    classes: 'w-full py-3.5 rounded-2xl text-sm font-bold text-white bg-green-500 hover:bg-green-600 disabled:opacity-50 transition-colors flex items-center justify-center gap-2 border-0 cursor-pointer',
  ...
  Component _buildRenterProfileModal(UserProfile profile, bool isDark) {
    return div(
      ...
      [
        div(
          classes: 'w-full max-w-md rounded-3xl shadow-2xl p-6 relative '
              '${isDark ? "bg-zinc-900 border border-zinc-800 text-white" : "bg-white text-zinc-900"}',
          [
            // Close button
            button(
              classes: 'absolute top-4 right-4 p-2 rounded-full hover:bg-zinc-800/20 transition-colors bg-transparent border-0 cursor-pointer',
            ...
            div(classes: 'flex flex-col items-center text-center space-y-4', [
              div(
                classes: 'w-20 h-20 rounded-full bg-purple-500/20 flex items-center justify-center overflow-hidden border-2 border-purple-500/40',
                [
              ...
              // Rating / Stats
              div(classes: 'flex items-center gap-4 py-2 border-y border-zinc-200 dark:border-zinc-800 w-full justify-around', [
                div(classes: 'text-center', [
                  div(classes: 'flex items-center gap-1 justify-center', [
                    lIcon('star', cls: 'w-4 h-4 text-yellow-400 fill-current'),
                    span(classes: 'font-bold text-sm', [Component.text(profile.rating != null ? profile.rating!.toStringAsFixed(1) : '5.0')]),
                  ]),
              ...
              div(classes: 'w-full text-left space-y-2.5 pt-2', [
                h4(classes: 'text-xs font-bold text-zinc-500 uppercase tracking-wider', [Component.text('Trust & Verification')]),
