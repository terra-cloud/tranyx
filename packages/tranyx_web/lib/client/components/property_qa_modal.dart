  ...
  Component build(BuildContext context) {
    ...
    return div(classes: 'fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm', [
      div(classes: 'w-full max-w-3xl max-h-[85vh] rounded-3xl border shadow-2xl flex flex-col animate-scale-up $bgCls', [
        ...
        // Header
        div(classes: 'p-6 border-b flex items-center justify-between ${isDark ? "border-zinc-800" : "border-zinc-100"}', [
          div([
        ...
        div(classes: 'flex-1 overflow-y-auto p-6 space-y-6', [
          if (_isLoading)
            ...
            div(classes: 'text-center p-10', [
              ...
              p(classes: 'text-sm $subTextCls mt-1', [Component.text('Be the first to ask the host a question about this property.')]),
            ])
          else
            for (final q in _questions)
              div(classes: 'p-5 rounded-2xl border ${isDark ? "border-zinc-800 bg-zinc-900/50" : "border-zinc-200 bg-zinc-50"}', [
                
                // Question
                div(classes: 'flex gap-3 mb-4', [
                  if (q['photoUrl'] != null && q['photoUrl'].toString().isNotEmpty)
                    img(src: q['photoUrl'].toString(), classes: 'w-10 h-10 rounded-full object-cover shrink-0', attributes: {'alt': 'User'})
                  else
                    div(classes: 'w-10 h-10 rounded-full bg-purple-500/20 flex items-center justify-center shrink-0', [
                  ...
                  div(classes: 'ml-13 mt-2', [
                    if (_answeringQuestionId == q['id'])
                      div(classes: 'flex flex-col gap-2', [
                        textarea(
                          classes: 'w-full p-3 rounded-xl text-sm border focus:border-purple-500 outline-none transition-colors ${isDark ? "bg-zinc-950 border-zinc-700 text-white" : "bg-white border-zinc-300 text-zinc-900"}',
                        ...
                        div(classes: 'flex justify-end gap-2', [
                          button(
                            classes: 'px-4 py-1.5 rounded-lg text-xs font-bold ${isDark ? "hover:bg-zinc-800" : "hover:bg-zinc-200"} cursor-pointer bg-transparent border-0',
                            events: {'click': (_) => setState(() { _answeringQuestionId = null; _answerText = ''; })},
                            [Component.text('Cancel')]
                          ),
                          button(
                            classes: 'px-4 py-1.5 rounded-lg text-xs font-bold text-white bg-purple-500 hover:bg-purple-600 cursor-pointer border-0',
                      ...
                      button(
                        classes: 'text-xs font-bold text-purple-400 hover:text-purple-300 flex items-center gap-1 bg-transparent border-0 cursor-pointer',
                        events: {'click': (_) => setState(() { _answeringQuestionId = q['id']; _answerText = ''; })},
        ...
        // Ask Input
        if (!isHost)
          div(classes: 'p-4 border-t ${isDark ? "border-zinc-800 bg-zinc-900" : "border-zinc-200 bg-zinc-50"} rounded-b-3xl', [
            div(classes: 'flex gap-2', [
              input(
                classes: 'flex-1 p-3 rounded-xl border focus:border-purple-500 outline-none transition-colors ${isDark ? "bg-zinc-950 border-zinc-700 text-white" : "bg-white border-zinc-300 text-zinc-900"}',
                attributes: {'placeholder': 'Ask a public question about this property...', 'value': _newQuestionText},
                events: {
              ...
              button(
                classes: 'px-6 py-3 rounded-xl text-white font-bold logo-gradient hover:opacity-90 transition-opacity cursor-pointer border-0',
